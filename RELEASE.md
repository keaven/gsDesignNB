...
This script will: 1. Extract the current version from the DESCRIPTION
file 2. Create an annotated tag with release notes from NEWS.md 3.
Prompt you to push the tag to GitHub 4. Once pushed, the GitHub Actions
workflow will automatically create the release
...
Once you push a tag matching the pattern `v*.*.*`, the
`.github/workflows/release.yaml` workflow will automatically: 1. Extract
the version number from the tag 2. Extract release notes from `NEWS.md`
for that version 3. Create a GitHub release with the extracted notes

You can monitor the workflow at:
<https://github.com/keaven/gsDesignNB/actions>
...
2.  **Automated release creation:**
    ...
    - Once the tag is pushed, the `.github/workflows/release.yaml`
...
**Major** (x.0.0): Incompatible API changes - **Minor** (0.x.0): New
functionality in a backward-compatible manner - **Patch** (0.0.x):
...
3.  Submit to CRAN via [CRAN submission
