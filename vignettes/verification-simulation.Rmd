...
and power for this complex design with piecewise accrual, dropout, and event gaps.

**Note on the event gap correction.**
When both overdispersion ($k > 0$) and an event gap ($g > 0$) are present, the naive effective rate formula
$\lambda/(1+\lambda g)$ overestimates the true population-level effective rate due to Jensen's inequality
(the function $f(x) = x/(1+xg)$ is concave in $x$, and subject-level rates vary due to frailty).
To address this, `sample_size_nbinom()` applies a second-order Taylor correction:
$$\lambda_{\text{eff}} \approx \frac{\lambda}{1+\lambda g}\left(1 - \frac{k\lambda g}{(1+\lambda g)^2}\right)$$
This reduces the error in the effective rate from approximately 7--8\% (naive) to approximately 1\% (corrected) in this scenario.
See `vignette("sample-size-nbinom")` for details.

Note also that the simulation results presented here were generated using the *uncorrected* sample size ($n = 422$).
With the Jensen correction, the required sample size increases to approximately 436 for this scenario, which would
improve power slightly above the nominal 90\% target.
