...
The simulation uses the corrected sample size ($n = 436$) computed by `sample_size_nbinom()`, which includes the
second-order Taylor correction for Jensen's inequality in the event gap effective rate
(see `vignette("sample-size-nbinom")`).
Results for the first $n = 422$ subjects are also extracted from each simulation to compare
with the uncorrected (naive) sample size.
...
*   **Event gap**: 20 days.
*   **Accrual**: Piecewise ramp-up over 12 months (Rate $R$ for 0-6mo, $2R$ for 6-12mo).
...
We simulated 3,600 trials at the corrected sample size ($n = `r design$n_total`$).
For each trial, we also analyzed the first $n = 422$ subjects to provide a comparison with the 
naive (uncorrected) sample size.
This number of simulations achieves a standard error for the power estimate 
...
The key question is whether the corrected sample size achieves the nominal 90% power.

```{r power_comparison, eval=!is.null(results)}
...
We compare the theoretical predictions from `sample_size_nbinom()` with the observed simulation results at the corrected sample size.
...
summary_df$Group <- factor(
  ifelse(summary_df$Metric == "Power", "Power",
    ifelse(summary_df$Metric == "Variance of log(RR)", "Variance",
      ifelse(summary_df$Metric == "Treatment Effect: log(RR)", "Treatment Effect",
        ifelse(grepl("Events", summary_df$Metric), "Events",
          ifelse(grepl("Exposure", summary_df$Metric), "Exposure", NA_character_))))),
...
```{r z_score_comparison, eval=!is.null(results) && "z_score" %in% names(results)}
...
A well-calibrated test should reject at most $\alpha = 0.025$ under the null ($\text{RR} = 1$).
We simulated trials under the null hypothesis using the same design parameters
(accrual, dropout, dispersion, sample size) but with $\lambda_2 = \lambda_1$ in both arms.
The Wald test (based on the ML estimate under the alternative) tends to be slightly
anti-conservative in finite samples, while the score test (evaluated entirely under $H_0$)
is expected to be better calibrated.
...
```{r type1_z_density, eval=has_null}
...
The Jensen correction for event gaps only applies when both the dispersion ($k > 0$) and the event gap ($g > 0$) are nonzero.
When either is zero, the corrected and naive formulas produce identical sample sizes.

To assess the correction's impact across a range of realistic parameter combinations, we ran a simulation sweep with 
10,000 replicates per scenario. For each scenario, trials were simulated at the corrected sample size ($n_\text{corr}$), 
and results were also extracted for the first $n_\text{naive}$ subjects (the uncorrected sample size). 
Because both analyses use the same simulated data, the **paired** comparison is very precise.
...
```{r sweep_interpretation, eval=has_sweep, results='asis'}
cat("**Key findings:**\n\n")
...
In the primary verification scenario above ($k = 0.5$, gap = 20 days), the corrected design slightly overshoots
the 90% target while the naive design is very close to 90%.
This pattern --- where the naive formula appears well-calibrated --- reflects a **partial cancellation of two biases**:

1.  **Design-stage bias** (hurts power): The naive effective rate $\lambda/(1+\lambda g)$ overestimates the true
    population-level effective rate, leading to an underestimated variance and thus a sample size that is too small.
    
2.  **Analysis-stage bias** (helps power): The model-based standard error from `MASS::glm.nb()` tends to
    slightly underestimate the true standard deviation of $\hat\theta$, making the Wald test slightly
    anti-conservative (z-statistic too extreme, more rejections).

These biases act in opposite directions.
In the $k = 0.5$, gap = 20 day scenario, they happen to nearly cancel, giving the naive design the appearance of correct calibration.

We apply the Jensen correction despite this for several reasons:

*   **The cancellation is fragile.** It depends on the specific test statistic (NB Wald via `glm.nb`). A different analysis method --- sandwich standard errors, bootstrap, permutation test, or a different link function --- would change the analysis-stage bias while the design-stage bias remains, breaking the cancellation.

*   **The correction is conservative.** It produces power at or slightly above 90% rather than at or slightly below.  For a pivotal trial, a slight overshoot of the power target is preferable to a slight undershoot.

*   **The correction is principled.** It fixes the effective rate approximation by accounting for the known distributional assumption (Gamma frailty). The remaining small overshoot is a separate finite-sample phenomenon.

*   **The scenario sweep demonstrates the benefit.** As the table above shows, the naive formula increasingly underpowers as $k$ and $g$ grow, while the corrected formula maintains nominal or conservative power across all scenarios tested.

*   **The cost is modest.** The extra subjects required by the correction range from 14 to 40 (3--6% of total), a small price for reliable power.
