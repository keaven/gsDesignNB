...
The function supports specifying `dropout_rate` and `max_followup` as vectors of length 2, corresponding to the control and treatment groups respectively.
When these differ, the average exposure $\bar{t}_g$, the second moment $\mathrm{E}[t_g^2]$, and the variance inflation factor $Q_g$ are all computed separately for each group.
