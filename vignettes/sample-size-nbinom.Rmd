...
The sample size formula is based on the asymptotic normality of the maximum likelihood estimator of the log rate ratio $\hat\theta$, using a Wald test.
...
The sample size formula implemented in `sample_size_nbinom()` corresponds to **Method 3** of @zhu2014sample, which is the Wald test for the log rate ratio under a negative binomial model with a log link and an exposure offset.
...
The target information for a fixed design is:
