...
4. **Simulation and SSR:** Operating characteristics should be reported for both **fixed GSD** and **SSR** pipelines, including recurrent-event features such as **seasonality** or event thinning; these are the principal **external** checks on complex code paths. For a software-journal submission, the manuscript should report compact operating-characteristic tables, Monte Carlo standard errors, and interpretation, while the package vignettes serve as executable supplementary material containing the full grids, code, and rendered output. In the vignette example, blinded SSR tended to request larger adapted sample sizes than unblinded SSR, and a slightly smaller nominal alpha improved Type I calibration.

5. **Mutze-type interim tests and dispersion:** For recurrent **count** outcomes, the **Mutze et al.** Wald test implemented in **gsDesignNB** as `mutze_test()` fits a negative binomial log-linear model with an offset for exposure. The fallback rule is now two-sided. If the fitted NB shape is very large (\(\theta > 50\) by default, equivalently \(\hat{k}<0.02\)), the data are practically Poisson and the implementation uses a Poisson Wald test. If the fit implies extreme overdispersion (\(\theta < 1/20\) by default) or NB ML fails in an overdispersed setting, the implementation re-estimates the nuisance rates and dispersion by method of moments and uses the NB Fisher information formula, rather than collapsing to a Poisson variance. Sensitivity analyses across `poisson_threshold`, `mom_threshold`, or explicit `method = "poisson"` versus `"nb"` should accompany any change from package defaults.

6. **Method of moments, dropout, and delivery layers:** Group-sequential simulation in **gsDesignNB** (`sim_gs_nbinom()`) records **maximum-likelihood** and **method-of-moments** dispersion estimates and the associated blinded and unblinded **information** paths. If ML-based nuisance estimates behave poorly at small interim sample sizes, comparing MoM and ML paths in simulation supports whether MoM is preferable for **SSR** or information monitoring. SSR accounts for dropout through observed interim exposure and through planned dropout assumptions in the recalculated sample size. MNAR imputation is best treated as a future sensitivity-analysis extension requiring censoring reason, remaining planned follow-up, and prespecified post-dropout event-rate assumptions; the controlled/reference-based recurrent-event imputation approach of Keene, Roger, Hartley, and Kenward (2014) is the natural literature template. Validated package APIs should remain primary; optional **Shiny** and **AI** layers are useful only when they sit above the same tested statistical core.
...
| Mutze Wald test (Poisson near \(k=0\), MoM NB fallback under extreme overdispersion) | `mutze_test()` (**gsDesignNB**) |
...
Possible outlets: *Statistics in Medicine*, *Biometrics*, *Pharmaceutical Statistics*, *Clinical Trials*, or a **software-focused** track (e.g., *Journal of Statistical Software*) if emphasis shifts to implementation. For a JSS-style submission, the package articles should be cited as supplementary material for the detailed simulation grids and code.
