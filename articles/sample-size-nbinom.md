...
with mean $`\mu`$ and dispersion parameter $`k`$, such that the variance
is given by:
...
We wish to test the null hypothesis $`H_0: \lambda_1 = \lambda_2`$
against the alternative $`H_1: \lambda_1 \neq \lambda_2`$, where
...
The sample size for the control group ($`n_1`$) is:
...
where $`r = n_2/n_1`$ is the allocation ratio, and
$`\mu_i = \lambda_i \cdot t`$ is the expected mean count over exposure
duration $`t`$.
...
where $`p_1 = n_1/n_{total}`$ and $`p_2 = n_2/n_{total}`$ are the
...
2.  The start time of segment $`j`$ is
    $`S_{j-1} = \sum_{i=1}^{j-1} D_i`$ (with $`S_0 = 0`$).
...
1.  **All truncated:** If $`u_{min} \ge F`$, all patients in the segment
    have potential follow-up $`\ge F`$, so their actual follow-up is
    $`F`$ (subject to dropout).
2.  **None truncated:** If $`u_{max} \le F`$, no patients reach the cap
...
$`k_{adj} = k \cdot Q`$. The function automatically calculates $`E[t]`$
and $`E[t^2]`$ based on the accrual, dropout, and trial duration
parameters.
