...
Suppose we have a control group with an event rate \lambda_1 = 0.1. We
assume the new treatment has the same event rate \lambda_2 = 0.09. We
want to rule out the possibility that the new treatment increases the
rate by more than 10% (i.e., a rate ratio of 1.1).
...
Suppose we have a control group with an event rate \lambda_1 = 0.1 and a
highly effective new treatment with \lambda_2 = 0.02. For instance, if
the population is healthy we may want to show a large reduction in event
rates for a new preventive treatment that may have side effects. This is
in line with vaccine trials where large efficacy margins are often
required. We wish to test if the risk reduction is greater than 50%
(i.e., the rate ratio is less than 0.5).
...
Here, we are testing H_0: \lambda_2 / \lambda_1 \ge 0.5 against H_1:
\lambda_2 / \lambda_1 \< 0.5. Since the assumed effect
(\lambda_2/\lambda_1 = 0.2) is much stronger than the threshold (0.5),
the power is high or the required sample size is relatively small
